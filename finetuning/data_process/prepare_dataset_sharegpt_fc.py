...
def get_convfinqa_dataset(json_file, is_test=False):
    ...
    system_prompt = """You are an expert in function calling on financial data. 
You are given a question and a set of possible tools. You must decide if a tool should be invoked. 
...
def prepare_fc_datasets(data_dir="data", output_dir="processed_data", include_test=False):
    """
    Prepare ConvFinQA function calling datasets for training, validation, and testing.
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Prepare ConvFinQA multi-turn datasets for function calling training")
    ...
    dataset_out = prepare_fc_datasets(
